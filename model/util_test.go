...
func TestIdent(t *testing.T) {
	tests := []struct {
		...
		{"MySQL 8 analytic function", []string{"rank"}, "`rank`"},
		{"row_number reserved", []string{"row_number"}, "`row_number`"},
		{"lateral reserved", []string{"lateral"}, "`lateral`"},
		{"cume_dist reserved", []string{"cume_dist"}, "`cume_dist`"},
		{"non-reserved keyword passes through", []string{"name"}, "name"},
		{"non-reserved 'status' passes through", []string{"status"}, "status"},
