...
// mysqlReservedWords lists MySQL 8.0 *reserved* keywords (the ones MySQL
// rejects as bare identifiers). Non-reserved keywords like NAME / STATUS /
// LANGUAGE are intentionally absent — MySQL itself accepts them as bare
// identifiers, and quoting them in dumps would produce noisy output.
//
// Source: https://dev.mysql.com/doc/refman/8.0/en/keywords.html (Reserved
// column). Updated against the 8.0 list; additions over time may be
// merged in.
...
dec decimal declare default delayed delete dense_rank desc describe deterministic distinct distinctrow div double drop dual
...
join json_table
key keys kill
lag last_value lateral lead leading leave left like limit linear lines load localtime localtimestamp lock long longblob longtext loop low_priority
...
when where while with window write
xor
year_month
zerofill
`)
...
// Empty parts are skipped. Names that aren't safe-unquoted or that collide
// with a MySQL reserved word are quoted.
