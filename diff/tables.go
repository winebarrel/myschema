...
func diffTable(current, desired *model.Table, dc DropChecker) (*tableDiffResult, error) {
	...
	// v1 partition support is round-trip + drift-detect only — both
	// sides normalise through `parser.NormalizePartitionOption` so
	// `dump → plan` reports no diff for an unchanged partitioned
	// table, but anything that differs (added partitions, removed
	// partitions, scheme change) errors out instead of emitting an
	// ALTER. ADD/DROP/TRUNCATE/REORGANIZE PARTITION generation is a
	// future PR; users manage partition changes by hand for now.
	if !ptrEq(current.Partition, desired.Partition) {
		return nil, fmt.Errorf("table %s: partition definition differs between catalog and desired SQL; partition diffs are not yet generated by myschema (v1 supports round-trip only). Run the ALTER TABLE … {ADD|DROP|TRUNCATE|REORGANIZE} PARTITION by hand and let the next plan reconverge", fqtn)
	}
