...
func TestAppendAlterHints(t *testing.T) {
	cases := []struct {
		...
		// TRUNCATE PARTITION and EXCHANGE PARTITION aren't
		// generated by myschema today, but they're in
		// `partitionOpKeywords` because MySQL's parser rejects
		// the trailing-comma form on every partition op. Pin
		// the leading splice so a typo in the keyword list
		// doesn't silently fall back to trailing-comma (which
		// MySQL would 1064 on).
		{
			name:      "TRUNCATE PARTITION: leading-position splice",
			in:        "ALTER TABLE shop.events TRUNCATE PARTITION p1;",
			algorithm: "INPLACE",
			lock:      "NONE",
			want:      "ALTER TABLE shop.events ALGORITHM=INPLACE, LOCK=NONE, TRUNCATE PARTITION p1;",
		},
		{
			name:      "EXCHANGE PARTITION: leading-position splice",
			in:        "ALTER TABLE shop.events EXCHANGE PARTITION p1 WITH TABLE shop.swap;",
			algorithm: "INPLACE",
			lock:      "NONE",
			want:      "ALTER TABLE shop.events ALGORITHM=INPLACE, LOCK=NONE, EXCHANGE PARTITION p1 WITH TABLE shop.swap;",
		},
