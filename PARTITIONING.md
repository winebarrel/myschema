...
out of scope for myschema. Fails with the error
`partition strategy / expression differs`.
...
Both directions are out of scope for myschema. The error messages
match the operation so the remediation hints are targeted:

- desired adds `PARTITION BY` to an unpartitioned catalog table →
  error: `first-time ALTER TABLE … PARTITION BY … is not yet generated by myschema`
  (the wording is historical — there is no roadmap PR for this).
- desired drops the `PARTITION BY` clause from an
  already-partitioned catalog table →
  error: `ALTER TABLE … REMOVE PARTITIONING is not yet generated by myschema`
  (same caveat).
