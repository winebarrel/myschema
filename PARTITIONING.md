...
out of scope for myschema. Fails with `partition strategy /
expression differs`.
...
- desired adds `PARTITION BY` to an unpartitioned catalog table →
  ...
  myschema` (the wording is historical — there is no roadmap PR
  for this).
- desired drops the `PARTITION BY` clause from an
  already-partitioned catalog table → `ALTER TABLE … REMOVE
  PARTITIONING is not yet generated by myschema` (same caveat).
