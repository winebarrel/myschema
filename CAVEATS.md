...
## Partitioning is round-trip only in v1
...
- Generating `ALTER TABLE … {ADD|DROP|TRUNCATE|REORGANIZE}
  PARTITION` from a desired-vs-catalog diff. Any partition
  difference (added / dropped / reshaped partitions, changed
  scheme, changed expression) errors out at plan time with
  `partition definition differs between catalog and desired
  SQL; partition diffs are not yet generated by myschema (v1
  supports round-trip only)`. The user is expected to run the
  partition DDL by hand and let the next plan reconverge.
- `SUBPARTITION BY …` is out of scope for v1. A desired-side
  `CREATE TABLE` that declares SUBPARTITION fails at parse
  time with `SUBPARTITION BY … is out of scope for v1
  partition support`. A catalog-side table that already has
  SUBPARTITION is rejected when the partition clause is read
  back, so myschema won't try to manage it. Drop the
  SUBPARTITION (manually) before bringing the table under
  myschema's management, or manage it outside myschema.

**Why round-trip first.** Each partition strategy has its own
`ALTER TABLE … PARTITION` syntax (RANGE/LIST take an
explicit `INTO (PARTITION … VALUES …)` reorganise; HASH/KEY use
`COALESCE PARTITION` for shrinks and bare `ADD PARTITION` for
grows; SUBPARTITION reshape is its own grammar) and the safe
order of operations depends on data layout. Shipping
"`dump → plan` is clean for partitioned tables" first lets users
adopt myschema for the rest of their schema without having to
hand-edit the partition clause out of every desired SQL file;
the diff generation can land later as a separate PR.

**Workaround for the "two systems of record" problem.** If you
need to add / drop partitions while myschema is in charge of
the table, run the `ALTER TABLE … {ADD|DROP|TRUNCATE|REORGANIZE}
PARTITION` by hand against the live database, then update the
desired SQL's `PARTITION BY` clause to match. The next `plan`
will report no diff. Keep the desired SQL and the live database
in lockstep yourself for now.
