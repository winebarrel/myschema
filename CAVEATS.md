...
- *Dropping a column the partition expression depends on* —
  MySQL itself returns `Error 3855: Column ... has a
  partitioning function dependency and cannot be dropped or
  renamed`. When `--allow-drop=column` is set and the
  partition-key column is absent from the desired CREATE
  TABLE, myschema surfaces this at plan time as
  `column drop(s) … conflict with the partition expression`
  rather than emit an executable-looking DROP COLUMN that
  the apply step would always reject. Workaround: remove
  the partition function dependency by hand first
  (`ALTER TABLE … REMOVE PARTITIONING` followed by a new
  `PARTITION BY …` against the surviving columns), then
  re-run plan. Renaming a partition-key column is also
  blocked by Error 3855, but a `-- myschema:renamed-from`
  on such a column inherently changes the desired-side
