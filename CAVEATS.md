...
- *Renaming or dropping a column the partition expression
  depends on* — MySQL itself returns `Error 3855: Column ...
  has a partitioning function dependency and cannot be
  dropped or renamed`. myschema surfaces the conflict at
  plan time so the user sees the failure before any DDL
  runs:
  - `RENAME COLUMN` on a partition-key column →
    `column rename(s) … conflict with the partition
    expression`.
  - `DROP COLUMN` on a partition-key column (the column is
    absent from the desired CREATE TABLE) →
    `column drop(s) … conflict with the partition expression`.
  Workaround for both: remove the partition function
  dependency by hand first (`ALTER TABLE … REMOVE
  PARTITIONING` followed by a new `PARTITION BY …` against
  the renamed / surviving columns), then re-run plan.
